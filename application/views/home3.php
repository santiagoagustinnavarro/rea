<!-- Page Content -->
<div class="container">



<!-- Portfolio Item Row -->
<div class="row">
<div id="titulo"></div>
  <div class="col-md-8" id="player">
    <img class="img-fluid" src="http://placehold.it/750x500" alt="">
  </div>

  <div class="col-md-4">
    <h3 class="my-3">Rea</h3>
    <p>Rea (Recursos Educativos abiertos) es una plataforma diseñada para profesores, para ayudarlos proporcionando material didactico para dar clases con temas referidos a la informatica</p>
    <h3 class="my-3">Aqui encontrara</h3>
    <ul>
      <li>Material didactico</li>
      <li>Archivos de distinto tipo</li>
      <li>Posibilidad de descarga de los archivos</li>
      <li>Orden cronologico de clases</li>
    </ul>
  </div>

</div>
<!-- /.row -->

<!-- Related Projects Row -->
<h3 class="my-4">Recursos mejor valorados</h3>

<div class="row">

  <div class="col-md-3 col-sm-6 mb-4">
    <a href="#">
      <img class="img-fluid" src="http://placehold.it/500x300" alt="">
    </a>
  </div>

  <div class="col-md-3 col-sm-6 mb-4">
    <a href="#">
      <img class="img-fluid" src="http://placehold.it/500x300" alt="">
    </a>
  </div>

  <div class="col-md-3 col-sm-6 mb-4">
    <a href="#">
      <img class="img-fluid" src="http://placehold.it/500x300" alt="">
    </a>
  </div>

  <div class="col-md-3 col-sm-6 mb-4">
    <a href="#">
      <img class="img-fluid" src="http://placehold.it/500x300" alt="">
    </a>
  </div>

</div>
<!-- /.row -->

</div>
<!-- /.container -->
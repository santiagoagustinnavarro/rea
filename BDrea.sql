...
CREATE TABLE IF NOT EXISTS `Rea`.`tokenRecupera` (
  `idToken` VARCHAR(70) NOT NULL,
  `estadoToken` VARCHAR(45) NULL,
  `nombreUsuario` VARCHAR(30) NOT NULL,
  PRIMARY KEY (`idToken`),
...
CREATE TABLE IF NOT EXISTS `Rea`.`tenerEstadoToken` (
  `idToken` VARCHAR(70) NOT NULL,
  `nombreEstadoToken` VARCHAR(15) NOT NULL,
  PRIMARY KEY (`idToken`, `nombreEstadoToken`),
  INDEX `fk_tokenRecupera_has_estadoToken_estadoToken1_idx` (`nombreEstadoToken` ASC),
  INDEX `fk_tokenRecupera_has_estadoToken_tokenRecupera1_idx` (`idToken` ASC),
  CONSTRAINT `fk_tokenRecupera_has_estadoToken_tokenRecupera1`
    FOREIGN KEY (`idToken`)
    REFERENCES `Rea`.`tokenRecupera` (`idToken`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
